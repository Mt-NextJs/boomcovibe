...
export default function ScheduleList({ flag, data }: scheduleProps) {
    const { paramsId, block } = useBlockSubmit();
    ...
    const handleDelete = async (itemId) => {
        ...
        const updatedSchedule = block.schedule.filter(
            (item) => item.id !== itemId,
        );
                ...
                <>
                    <section className="mb-4 flex h-40 w-full cursor-default justify-center gap-1">
                        ...
                        <div className="flex h-40 w-1/6 flex-col items-center justify-evenly gap-4">
                            <Link
                                href={`/admin/block/calendar/form?paramsId=${paramsId}&schedule=${data.id}`}
                            >
                            ...
                            <button
                                className="rounded-xl bg-red-100 px-5 py-3.5 font-bold text-red-500 hover:bg-red-300"
