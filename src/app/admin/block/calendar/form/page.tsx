...
export default function CalendarFormBlock() {
    ...
    const parsedParamsId = searchParams.get('paramsId');
    const editing = searchParams.get('schedule');
    // paramsId를 숫자로 변환
    const paramsId = parsedParamsId ? Number(parsedParamsId) : null;
    ...
    useEffect(() => {
        if (editing && block?.schedule) {
            const targetData = block.schedule.find(
                (schedule) => schedule.id === Number(editing),
            );
            if (targetData) {
                setScheduleName(targetData.title);
                setLinkAddress(targetData.url);
            }
        }
    }, [editing]);
    ...
    return (
        <>
            <BlockHeader
                ...
                iconLink={
                    paramsId
                        ? `/admin/block/calendar?id=${paramsId}`
