...
export default function CalendarBlock() {
  ...
  useEffect(() => {
    blocks ? setDataSet(true) : setDataSet(false);
  }, [blocks]);
  return (
    <>
      <BlockHeader
      ...
      <div className="px-10">
        <Link href={"/admin/block/calendar/form"}>
          <button className="button color">+ 캘린더에 일정을 추가하세요</button>
        </Link>
      </div>
                ...
                <div
                  ...
                  <div
                    className={`button-color h-3 w-3 rounded-full ${
                      isListView ? "border-background border-2 bg-primary" : ""
                    }`}></div>
                </div>
                <div>리스트뷰</div>
              </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="mb-5 px-10 py-5 h-[20rem] overflow-y-scroll [&::-webkit-scrollbar]:hidden">
          {viewTypeNow === true ? (
            //  데이터세트 들어오면 로직 수정
            dataSet === true ? (
