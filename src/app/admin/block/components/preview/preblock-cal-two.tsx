    ...
    renderBlock?.forEach((schedule: SchedulePlus) => {
      ...
      if (
        ...
        if (day >= startDate.getDate() && day <= endDate.getDate()) {
          ...
          blocksToRender.push(
            <div key={`${schedule.id}-block`} className={blockClass}>
              {isStart ? <p className="my-auto">{schedule.title}</p> : null}
            </div>
          );
        } else if (day >= startDate.getDate() && day > endDate.getDate()) {
          blocksToRender.push(
            <div
              key={`${schedule.id}-block`}
              className="bg-white my-2 h-6"></div>
          );
        }
      }
    });
                    ...
                    {day ? (
                      <>
                        <div className="px-3 pt-4">
                          <div
                            className={`${
                              day === todayDay ? "bg-primary text-white" : ""
                            } flex h-6 w-6 items-start justify-center rounded-full text-base`}>
                            {day}
                          </div>
                        </div>
                        {renderBlocksForRange(day, weekIndex)}
                      </>
                    ) : (
